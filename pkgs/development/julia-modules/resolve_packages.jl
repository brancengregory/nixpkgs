...
# Only resolve weak dependencies if explicitly requested
if VERSION >= VersionNumber("1.9") && resolve_weak_dependencies
    println("Weak dependency resolution is ENABLED - this may cause version conflicts")
    while true
...
elseif VERSION >= VersionNumber("1.9")
    println("Weak dependency resolution is DISABLED - only resolving direct dependencies")
end
